...
#
# Available variables:
# CONFIG_NAME            Instance name (config filename without .conf)
# CONFIG_PATH            Absolute path to the config file
# SOURCE_DIR             Source directory being watched
# TARGET_DIR             Target directory to copy files to
# TARGET_OWNER           Owner:group for target files (optional)
# EVENTS                 Comma-separated inotify events
# RECURSIVE              Whether subdirs are watched
...
# ACTION_SCRIPT          Script used to process files
# PRE_HOOK_DIR           Directory with pre-copy hooks
# POST_HOOK_DIR          Directory with post-copy hooks
# POST_HOOK_BATCH_SIZE   Max number of files before flushing batch
# POST_HOOK_IDLE_TIMEOUT Max idle seconds before flushing batch
# MAIL_ON_ERROR          Mail address to notify on errors
```
